...
class Robot(RobotBase):
    def __init__(self):
        ...
        self.motor_left_speed = self.attach_device(SpeedEncoder(8, 25, reverse=True))
        self.motor_left_driver = SpeedControlledMotor(self.motor_left, self.motor_left_speed, PID.PID())
        self.motor_right = self.attach_device(Motor(24, 23, 18, reverse=True))
        self.motor_right_speed = self.attach_device(SpeedEncoder(17, 27, reverse=True))
        self.motor_right_driver = SpeedControlledMotor(self.motor_right, self.motor_right_speed, PID.PID())
